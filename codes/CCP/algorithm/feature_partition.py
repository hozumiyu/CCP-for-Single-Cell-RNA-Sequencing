...
def partition_features(X, n_components, partition_method, partition_metric, random_state):
    ...
    if partition_method == 'kmeans':
        labels = feature_kmeans(X, n_components, random_state =random_state)
    elif partition_method == 'kmedoids':
        labels = feature_kmedoids(X, n_components, partition_metric = 'correlation', random_state = random_state)
