...
def load_X(data, inpath):
    inpath = inpath + data + '/'
    X = pd.read_csv(inpath + '%s_labels.csv'%(data))
...
def load_y(data, inpath):
    inpath = inpath + data + '/'
    y = pd.read_csv(inpath + '%s_data.csv'%(data))
...
def subSample(data, inpath, unique_labels):
    X = load_X(data, inpath)
    inpath = inpath + data + '/'
    y_record = pd.read_csv(inpath + '%s_full_labels.csv'%(data))
...
outpath = './subsample/'; makeFolder(outpath)
'''
data = 'GSE84133human4'
for idx in range(1, 5):
    ...
    if not os.path.exists(outpath + 'GSE84133_ss%d_X_ccp.npy'%(idx)):
        ...
        X_train, y_train_temp = subSample(data, inpath, unique_labels)
        ...
        for idx2 in range(1,5):
            if idx != idx2:
                ...
                X_test, y_test_temp = subSample(data, inpath, unique_labels)
...
X_ccp = np.load(outpath + 'GSE84133_ss%d_X_ccp.npy'%(idx))
n_neighbors_vec =  [5, 10, 15, 20]
for idx in range(4):
    myUMAP = UMAP(random_state = 1, n_neighbors = n_neighbors_vec[idx])
    X_umap = myUMAP.fit_transform(X_ccp)
    plot(X_umap, y, ax[0, idx])

perplexity_vec = [5, 10, 15, 30]
for idx in range(4):
    myTSNE = TSNE(random_state = 1, perplexity = perplexity_vec[idx])
    X_tsne = myTSNE.fit_transform(X_ccp)
    plot(X_tsne, y, ax[1, idx])
'''
...
fig.savefig('./plotting_code/figures/' + 'GSE84133_subsample.png', bbox_inches='tight', dpi = 500)
yindex = np.array([1512, 1559, 2490, 1223])
    
fig, ax = plt.subplots(2, 4, figsize = (8.5, 8.5/2))
for idx in range(1, 5):
    X_umap = np.load(outpath + 'GSE84133_ss%d_X_umap.npy'%(idx))
    y = np.zeros(X_umap.shape[0])
    start = 0
    end = yindex[idx-1]
    y[start:end] = 1
    for idx2 in range(1, 5):
